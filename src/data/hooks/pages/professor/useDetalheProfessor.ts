import { AlunoErroResponseInterface, AlunoInterface } from "@data/@types/aluno";
...
export default function useDetalheProfessor() {
  ...
  function formatDataJson(data: string): Date {
    // 10/10/2023 15:00
    // ["10/10/2023", "15:00"]
    // ["10", "10", "2023"]
  ...
  async function handleSubmit() {
    const newDate = {
      ...aluno,
      data_aula: formatDataJson(aluno.data_aula as string),
    } as AlunoInterface;

    await ApiService.post(`/api/professores/${professor?.id}/alunos`, newDate).then(
      () => {
        setOpenDialog(false);
